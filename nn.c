...
static void backpropagate(NN* network, double learning_rate,double** reference_vals, Matrix* layer_gradients1, Matrix* layer_gradients2 ){
    ...
    for (size_t i = 0; i < network->outputs->nb_rows ; i++)
    {
        ...
        {
            layer_gradients1->data[i][j] = network->outputs->data[i][j] - reference_vals[i][j]; 
        }
        // printf("bunitinho \n");
    }
    ...
    for (size_t i = 0; i < network->output_layer_weights->nb_cols; i++)
    {
        for (size_t j = 0; j < network->output_layer_weights->nb_rows; j++)
        {
            double adjustment = 0;
            
            for (size_t k = 0; k < network->batch_size; k++)
            {
                adjustment += get_last_layer(network)->values.data[k][j]* layer_gradients1->data[k][i];
            }

            network->output_layer_weights->data[j][i] -= learning_rate*(adjustment/network->batch_size);   
    ...
    //         {
    //             correction += get_last_layer(network)->values.data[j][i] * layer_gradients1->data[j][i];
    //         }
    ...
    for (size_t i = 0; i < get_last_layer(network)->values.nb_cols; i++)
    {
        ...
        {   
            ...
            for (size_t k = 0; k < network->batch_size; k++)
            {
                layer_gradients2->data[k][i] = network->output_layer_weights->data[i][j]*layer_gradients1->data[k][j];
            }
    ...
    // matrix_render(layer_gradients1);
    ...
    for (int layer_idx = network->hidden_layer_count - 1; layer_idx >= 0; layer_idx--)
    {
        ...
        for (size_t i = 0; i < gradients_size; i++)
        {
            ...
            {
                layer_gradients1->data[j][i] = layer->d_activation(layer->values.data[j][i]);
            }
        ...
        for (size_t i = 0; i < layer->biases.nb_cols; i++)
        {
            ...
            for (size_t j = 0; j < network->batch_size; j++)
            {
                bias_adjustment += layer_gradients1->data[j][i];
            }
        ...
        //adjusting weights
        for (size_t i = 0; i < layer->weights.nb_cols; i++)
        {
            for (size_t j = 0; j < layer->weights.nb_rows; j++)
            {
                double adjustment = 0;
                
                for (size_t k = 0; k < network->batch_size; k++)
                {
                    adjustment += previous_layer_activations->data[k][j]* layer_gradients1->data[k][i];
                }

                layer->weights.data[j][i] -= learning_rate*(adjustment/network->batch_size);   
        ...
        if(layer_idx > 0){
            ...
            for (size_t i = 0; i < previous_layer_activations->nb_cols; i++)
            {
                ...
                {   
                    ...
                    for (size_t k = 0; k < network->batch_size; k++)
                    {
                        layer_gradients2->data[k][i] = layer->weights.data[i][j]*layer_gradients1->data[k][j];
                    }
...
void NN_fit_classification(NN* network,size_t nb_examples ,size_t nb_epochs ,double** values, int* labels, double learning_rate){
    ...
    Matrix* layer_gradients1 =  matrix_new(network->batch_size, layer_gradients_size);
    Matrix* layer_gradients2 =  matrix_new(network->batch_size, layer_gradients_size);
    ...
    matrix_delete(batch);
    matrix_delete(layer_gradients1);
    matrix_delete(layer_gradients2);
